...
export default function ViewProfileScreen({ navigation, route }: Props) {
  ...
  const handleUnmatch = useCallback(() => {
    ...
    Alert.alert(
      'Unmatch',
      `Are you sure you want to unmatch with ${profile.name}? This cannot be undone.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              // Unmatch is handled via the matches API — for now we navigate back
              // In production, call unmatchUser(match_id) from matches API
              navigation.goBack();
            } catch {
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <View style={styles.errorContainer}>
          ...
          <Button title="Retry" onPress={fetchProfile} variant="outline" fullWidth={false} style={styles.retryButton} />
        </View>
      </SafeAreaView>
    );
  }
          ...
          <View style={styles.section}>
            <Card style={styles.card}>
              ...
              {profile.voice_prompts.map((vp) => (
                <TouchableOpacity key={vp.id} style={styles.voiceItem} activeOpacity={0.7}>
